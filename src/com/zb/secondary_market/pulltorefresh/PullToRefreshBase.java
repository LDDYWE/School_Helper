...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	public final boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP: {
			...
			if (isBeingDragged) {
				...
				Log.e("知乎", firstMotionY + " " + lastMotionY2);

				if (lastMotionY2 > firstMotionY) {
					Log.v("知乎", "200");
				}

				if (state == RELEASE_TO_REFRESH && null != onRefreshListener
				// &&lastMotionY2 > firstMotionY
				) {
					Log.v("知乎", "4");
					setRefreshingInternal(true);
					Log.v("知乎", "5");
					if(lastMotionY2 > firstMotionY){
					onRefreshListener.onRefreshU2D();
					}else{
						onRefreshListener.onRefreshD2U();
					}
					Log.v("知乎", "6");
				} else {
					Log.v("知乎", "7");
	...
	private void init(Context context, AttributeSet attrs) {
		...
		String pullLabel_u2d = context
				.getString(R.string.pull_to_refresh_pull_u2d_label);
		String refreshingLabel_u2d = context
				.getString(R.string.pull_to_refresh_refreshing_u2d_label);
		String releaseLabel_u2d = context
				.getString(R.string.pull_to_refresh_release_u2d_label);
		
		String pullLabel_d2u = context
				.getString(R.string.pull_to_refresh_pull_d2u_label);
		String refreshingLabel_d2u = context
				.getString(R.string.pull_to_refresh_refreshing_d2u_label);
		String releaseLabel_d2u = context
				.getString(R.string.pull_to_refresh_release_d2u_label);

		// Add Loading Views
		if (mode == MODE_PULL_DOWN_TO_REFRESH || mode == MODE_BOTH) {
			headerLayout = new LoadingLayout(context,
					MODE_PULL_DOWN_TO_REFRESH, releaseLabel_u2d, pullLabel_u2d,
					refreshingLabel_u2d);
		...
		if (mode == MODE_PULL_UP_TO_REFRESH || mode == MODE_BOTH) {
			footerLayout = new LoadingLayout(context, MODE_PULL_UP_TO_REFRESH,
					releaseLabel_d2u, pullLabel_d2u, refreshingLabel_d2u);
		...
		if (a.hasValue(R.styleable.PullToRefresh_headerTextColor)) {
			final int color = a.getColor(
					R.styleable.PullToRefresh_headerTextColor, Color.GRAY);
			if (null != headerLayout) {
